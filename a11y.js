...
(function (g) {
  ...
  function injectCSS() {
    ...
    var css = ""
      ...
      + ".a11y-title{flex:1;font-family:var(--a-display);font-size:1.35rem;color:var(--a-ink)}"
      + ".a11y-close{background:transparent;border:1.5px solid var(--a-line);color:var(--a-ink);border-radius:0;width:30px;height:30px;cursor:pointer;font-size:14px}"
      + ".a11y-close:hover{background:var(--a-accent);color:var(--a-surface);border-color:var(--a-accent)}"
      + ".a11y-row{margin:0 0 1rem}"
      + ".a11y-lbl{font-family:var(--font-mono);font-size:10px;letter-spacing:0.12em;text-transform:uppercase;color:var(--a-band);margin-bottom:0.5rem}"
      + ".a11y-opts{display:flex;flex-wrap:wrap;gap:0.4rem}"
      + ".a11y-opt{font-family:var(--a-ui);font-weight:600;font-size:12.5px;padding:7px 13px;border-radius:0;border:1.5px solid var(--a-line);background:transparent;color:var(--a-ink);cursor:pointer;transition:background .12s,color .12s,border-color .12s}"
      + ".a11y-opt:hover{border-color:var(--a-accent);color:var(--a-accent)}"
      + ".a11y-opt.on{background:var(--a-accent);border-color:var(--a-accent);color:var(--a-surface)}"
      + ".a11y-size{display:flex;align-items:center;gap:0.7rem}"
      + ".a11y-size input{flex:1;accent-color:var(--a-accent)}"
      + ".a11y-size .v{font-family:var(--font-mono);font-size:12px;min-width:44px;text-align:right;color:var(--a-ink)}"
      + ".a11y-info{font-size:0.85rem;line-height:1.6;color:var(--a-muted);border-top:1px solid var(--a-line);padding-top:0.85rem;margin-top:0.4rem}"
      + ".a11y-info a{color:var(--a-accent);font-weight:600}"
      + ".a11y-reset{font-family:var(--font-mono);font-size:11px;color:var(--a-muted);background:none;border:0;cursor:pointer;text-decoration:underline;padding:0}"
      + ".a11y-refresh{border-color:var(--a-accent);color:var(--a-accent);font-weight:700}"
      + ".a11y-refresh:hover{background:var(--a-accent);color:var(--a-surface)}"
